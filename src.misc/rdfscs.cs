...
public class RDFSCS {
	...
	private static void DoType(Entity type, KnowledgeModel model, TextWriter output) {
		...
		foreach (Statement t in model.Select(new Statement(null, rdfsDomain, type)))
			DoProperty(t.Subject, true, model, output);
		
		foreach (Statement t in model.Select(new Statement(null, rdfsRange, type)))
			DoProperty(t.Subject, false, model, output);
